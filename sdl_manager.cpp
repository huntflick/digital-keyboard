...
SDL_Manager::SDL_Manager() {
    ...
    spawnWindow("Audio Interface", 300, 100, 0);
    IMGUI_CHECKVERSION();
    ImGui::CreateContext();
...
void SDL_Manager::renderUI(const std::string& song, const std::string& instrument, int octave) {
    ...
    std::string songText = "Current Song: " + song;
    float textWidth = ImGui::CalcTextSize(songText.c_str()).x;
    std::string instrumentText = "Instrument: " + instrument;
    float instTextWidth = ImGui::CalcTextSize(instrumentText.c_str()).x;
    float larger = std::max(textWidth, instTextWidth);
    ImGui::SetNextWindowSize(ImVec2(larger + 30.0f, 0.0f));
